<?php
/**
 * Product card used in Archive pages (with grid/list view support).
 *
...
?>
<li <?php wc_product_class( 'product-card-item flex bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition-all duration-300 relative group dmd-archive-card', $product ); ?> data-category="<?php echo esc_attr( implode( ' ', wp_get_post_terms( $product_id, 'product_cat', ['fields' => 'slugs'] ) ) ); ?>">
    ...
    <!-- Main Content wrapper (for list view) -->
    <div class="dmd-archive-main-content flex flex-row items-center gap-4 flex-grow">
        <!-- Thumbnail -->
        <a href="<?php the_permalink(); ?>" class="block w-28 h-28 shrink-0 rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center group-hover:opacity-95">
    ...
    <!-- Action Buttons -->
    <div class="dmd-archive-actions mt-auto pt-3 space-y-2 shrink-0">
        <?php 
        echo apply_filters( 'woocommerce_loop_add_to_cart_link',
            sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" class="%s">%s</a>',
                esc_url( $product->add_to_cart_url() ),
                esc_attr( $product_id ),
                'button product_type_simple add_to_cart_button ajax_add_to_cart w-full py-2 px-3 rounded-xl bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-semibold transition flex items-center justify-center gap-1 shadow-sm',
                esc_html__( 'Add to cart', 'digital-mudir-dokan' )
            ),
            $product
        );
        ?>
        <a href="<?php echo esc_url( wc_get_checkout_url() . '?add-to-cart=' . $product_id ); ?>" class="w-full py-2 px-3 rounded-xl bg-white border border-emerald-800 text-emerald-800 hover:bg-emerald-50 text-xs font-semibold transition flex items-center justify-center">
